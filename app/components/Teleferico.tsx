...
export default function Teleferico({ startX, y, delay = 0, color = 'purple', scale = 1, screenWidth = 1400 }: TelefericoProps) {
  const s = Math.min(scale, 1.3);
  const travelDistance = screenWidth + 400;
  ...
  return (
    <motion.div
      ...
      initial={{ x: startX, y }}
      animate={{ x: [startX, startX + travelDistance] }}
      transition={{ duration: 25, delay, repeat: Infinity, ease: "linear" }}
    >
      <div className="relative" style={{ transform: `scale(${s})`, transformOrigin: 'top left' }}>
        {/* Cable */}
        <div className="absolute -top-32 left-0 w-48 h-1 bg-linear-to-r from-gray-600 via-gray-700 to-gray-600 shadow-lg rounded-full" />
        ...
        <motion.div
          ...
          transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
          style={{ boxShadow: `0 15px 40px ${selectedColor.shadow}` }}
        >
          {/* Techo */}
          ...
          <div className="absolute top-6 left-2 w-16 h-20 bg-linear-to-br from-blue-100 via-blue-200 to-blue-300 border-3 border-gray-700 opacity-85 rounded-lg overflow-hidden">
            <div className="absolute top-0 bottom-0 left-1/2 w-1 bg-gray-700 transform -translate-x-1/2" />
            <div className="absolute left-0 right-0 top-1/3 h-1 bg-gray-700" />
            <div className="absolute left-0 right-0 top-2/3 h-1 bg-gray-700" />
            <motion.div
          ...
          <div className={`absolute bottom-0 left-1/2 transform -translate-x-1/2 w-14 h-24 ${selectedColor.accent} rounded-t-2xl border-4 border-gray-900`}>
            ...
            <div className="absolute top-14 right-2 w-1.5 h-4 bg-gray-800 rounded-full" />
          </div>
          ...
          {/* Base */}
          <div className="absolute bottom-0 left-0 right-0 h-2 bg-gray-800 border-t-2 border-gray-900" />

          {/* Laterales */}
          <div className="absolute top-4 left-0 w-2 h-24 bg-linear-to-b from-gray-700 to-gray-800 border-r border-gray-900" />
          <div className="absolute top-4 right-0 w-2 h-24 bg-linear-to-b from-gray-700 to-gray-800 border-l border-gray-900" />
        </motion.div>
      </div>
    </motion.div>
  );
