...
interface PacenoBuildingoProps {
  x: number;
  y?: number;
}

export default function PacenoBuilding({ x, y = 200 }: PacenoBuildingoProps) {
  return (
    <motion.div
    ...
    >
      <div className="relative w-64 h-96">
        ...
        <div className="absolute -top-8 left-0 right-0">
          ...
          >
            ...
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="absolute bottom-0 h-3 w-8 bg-red-800 border-b-2 border-red-950"
                style={{ left: `${i * 12}%` }}
              />
            ))}
          </div>
        ...
        {/* Cuerpo principal del edificio - estilo colonial */}
        <div className="relative w-full h-full bg-linear-to-br from-orange-300 via-orange-400 to-orange-500 shadow-2xl border-4 border-orange-600">
            ...
            <div className="relative">
              ...
              <div className="w-full h-20 bg-linear-to-b from-amber-700 to-amber-900 rounded-t-lg border-4 border-amber-800">
                ...
                <div className="absolute top-2 left-2 right-2 h-12 border-2 border-gray-800 rounded">
                  ...
                  {/* Ornamentos */}
                  <div className="absolute top-1/2 left-0 right-0 h-0.5 bg-gray-900"></div>
                </div>
                ...
                <div className="absolute -bottom-8 left-1/2 transform -translate-x-1/2 w-16 h-24 bg-linear-to-b from-amber-800 to-amber-950 rounded-t-lg border-2 border-amber-900">
                  ...
                  <div className="absolute top-2 left-2 right-2 h-8 bg-blue-200 opacity-60 border border-amber-900">
                    <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-amber-900"></div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {[...Array(2)].map((_, i) => (
              ...
              >
                ...
                <div className="absolute inset-2 bg-linear-to-br from-blue-200 to-blue-300 opacity-70 rounded border-2 border-amber-900">
                  {/* Marco cruzado */}
                  <div className="absolute top-0 bottom-0 left-1/2 w-1 bg-amber-900 transform -translate-x-1/2"></div>
                  <div className="absolute left-0 right-0 top-1/2 h-1 bg-amber-900 transform -translate-y-1/2"></div>
                  
                  {/* Reflejo */}
                  <div className="absolute top-1 left-1 w-4 h-4 bg-white opacity-40 rounded"></div>
                </div>
                ...
                <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2 w-12 h-3 bg-red-700 rounded-b-lg">
                  {/* Flores */}
                  <motion.div
                    className="absolute -top-2 left-2 w-2 h-2 bg-pink-500 rounded-full"
                    animate={{ scale: [1, 1.2, 1] }}
                    transition={{ duration: 2, repeat: Infinity }}
                  />
                  <motion.div
                    className="absolute -top-2 left-5 w-2 h-2 bg-red-500 rounded-full"
                    animate={{ scale: [1, 1.2, 1] }}
          ...
          <div className="absolute bottom-16 left-0 right-0 flex justify-around px-4">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="relative w-12 h-16 bg-linear-to-br from-amber-700 to-amber-900 rounded-lg border-3 border-amber-800"
              >
                <div className="absolute inset-2 bg-linear-to-br from-blue-200 to-blue-300 opacity-70 rounded border border-amber-900">
                  <div className="absolute top-0 bottom-0 left-1/2 w-0.5 bg-amber-900"></div>
                  <div className="absolute left-0 right-0 top-1/2 h-0.5 bg-amber-900"></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-20 h-28 bg-linear-to-b from-amber-800 to-amber-950 rounded-t-2xl border-4 border-amber-900">
            {/* Arco superior */}
            <div 
              className="absolute -top-4 left-0 right-0 h-8 bg-amber-800 border-4 border-amber-900"
              style={{ borderRadius: '50% 50% 0 0' }}
            />
            
            {/* Ventanilla en la puerta */}
            <div className="absolute top-4 left-1/2 transform -translate-x-1/2 w-8 h-10 bg-blue-300 opacity-60 rounded border-2 border-amber-900">
              <div className="absolute inset-1 border border-amber-900 rounded"></div>
            </div>

            {/* Manija */}
            <div className="absolute top-16 right-2 w-2 h-3 bg-yellow-600 rounded-full"></div>
            
            {/* Detalles decorativos */}
            <div className="absolute bottom-4 left-2 right-2 h-0.5 bg-amber-700"></div>
            <div className="absolute bottom-2 left-2 right-2 h-0.5 bg-amber-700"></div>
          </div>
          ...
          {/* Sombras y detalles de profundidad */}
          <div className="absolute top-0 left-0 w-2 h-full bg-orange-700 opacity-50"></div>
          <div className="absolute top-0 right-0 w-2 h-full bg-orange-600 opacity-30"></div>
        </div>

        {/* Base/zócalo del edificio */}
        <div className="absolute bottom-0 left-0 right-0 h-8 bg-linear-to-b from-stone-600 to-stone-800 border-t-4 border-stone-700">
          ...
          {[...Array(6)].map((_, i) => (
            <div
              key={i}
              className="absolute top-0 h-full w-px bg-stone-900 opacity-50"
              style={{ left: `${i * 16}%` }}
            />
          ))}
        </div>
